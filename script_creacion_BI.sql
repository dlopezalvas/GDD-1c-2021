...
SELECT 'Compra', AxC.COD_ACCES, S.COD_SUCURSAL, YEAR(C.FECHA) AS ANIO, MONTH(C.FECHA) MES, AxC.PRECIO, SUM(CANTIDAD) AS CANTIDAD, NULL, NULL, NULL
...
GROUP BY AxC.COD_ACCES, S.COD_SUCURSAL, AxC.PRECIO, YEAR(C.FECHA), MONTH(C.FECHA)
COMMIT;

BEGIN TRANSACTION
/*VENTAS ACCESORIOS POR SUCURSAL POR CLIENTE POR MES*/
INSERT INTO [BREAKFAST_CLUB].[BI_HECHOS_ACCESORIOS]
SELECT 'Venta', AxF.COD_ACCES, S.COD_SUCURSAL, YEAR(F.FECHA) AS ANIO, MONTH(F.FECHA) MES, AxF.PRECIO, SUM(CANTIDAD) AS CANT, COD_CLIENTE, NULL, NULL
...
GROUP BY S.COD_SUCURSAL, COD_CLIENTE,  AxF.COD_ACCES, AxF.PRECIO, YEAR(F.FECHA), MONTH(F.FECHA)
COMMIT;
...
SELECT 'Compra', PC.COD_PC, COD_RAM, COD_DISCO, COD_GPU, COD_CPU, S.COD_SUCURSAL, NULL, YEAR(C.FECHA) AS ANIO, MONTH(C.FECHA) MES, PxC.PRECIO, SUM(CANTIDAD) AS CANTIDAD,  NULL, NULL
...
GROUP BY PC.COD_PC, COD_GPU,  COD_RAM, COD_DISCO, COD_CPU, S.COD_SUCURSAL, PxC.PRECIO, YEAR(C.FECHA), MONTH(C.FECHA)
...
BEGIN TRANSACTION
INSERT INTO [BREAKFAST_CLUB].[BI_HECHOS_PC]
SELECT 'Venta', PC.COD_PC, COD_RAM, COD_DISCO, COD_GPU, COD_CPU, S.COD_SUCURSAL, F.COD_CLIENTE, YEAR(F.FECHA) AS ANIO,  MONTH(F.FECHA) MES, PxF.PRECIO, SUM(CANTIDAD) AS CANTIDAD, NULL, NULL
...
GROUP BY PC.COD_PC, COD_GPU,  COD_RAM, COD_DISCO, COD_CPU, S.COD_SUCURSAL, PxF.PRECIO, YEAR(F.FECHA), MONTH(F.FECHA)
COMMIT;
