...
BEGIN
	...
	DECLARE @CANT_TOTAL_VENTAS DECIMAL(18,0) = (SELECT SUM(CANTIDAD) FROM [BREAKFAST_CLUB].BI_HECHOS_ACCESORIOS  
	WHERE TIPO_TRANSACCION = 'VENTA' AND COD_ACCESORIO = @COD_ACCESORIO AND COD_SUCURSAL = @COD_SUCURSAL)

	DECLARE @TIEMPO_VENTA DECIMAL(18,0) = (SELECT SUM(CANTIDAD * COD_TIEMPO) FROM [BREAKFAST_CLUB].BI_HECHOS_ACCESORIOS 
	WHERE TIPO_TRANSACCION = 'VENTA' AND COD_ACCESORIO = @COD_ACCESORIO AND COD_SUCURSAL = @COD_SUCURSAL)
	...
	DECLARE @TIEMPO_COMPRA DECIMAL(18,0) = (SELECT SUM(CANTIDAD * COD_TIEMPO) AS TOTAL FROM [BREAKFAST_CLUB].BI_HECHOS_ACCESORIOS 
	WHERE TIPO_TRANSACCION = 'COMPRA' AND COD_ACCESORIO = @COD_ACCESORIO AND COD_SUCURSAL = @COD_SUCURSAL AND COD_TIEMPO <= @MES_ANTERIOR)

	DECLARE @CANT_TOTAL_MES_ANTERIOR DECIMAL(18,0) = (SELECT SUM(CANTIDAD) FROM [BREAKFAST_CLUB].BI_HECHOS_ACCESORIOS
	WHERE TIPO_TRANSACCION = 'COMPRA' AND COD_ACCESORIO = @COD_ACCESORIO AND COD_SUCURSAL = @COD_SUCURSAL AND COD_TIEMPO <= @MES_ANTERIOR)
...
BEGIN
	...
	DECLARE @CANT_TOTAL_VENTAS DECIMAL(18,0) = (SELECT SUM(CANTIDAD) FROM [BREAKFAST_CLUB].BI_HECHOS_PC  
	WHERE TIPO_TRANSACCION = 'VENTA' AND COD_PC = @COD_PC AND COD_SUCURSAL = @COD_SUCURSAL)

	DECLARE @TIEMPO_VENTA DECIMAL(18,0) = (SELECT SUM(CANTIDAD * COD_TIEMPO) FROM [BREAKFAST_CLUB].BI_HECHOS_PC 
	WHERE TIPO_TRANSACCION = 'VENTA' AND COD_PC = @COD_PC AND COD_SUCURSAL = @COD_SUCURSAL)

	DECLARE @MES_ANTERIOR DECIMAL(18,0) = (SELECT MAX(P.COD_TIEMPO) FROM (SELECT ha.cod_tiempo AS COD_TIEMPO FROM   [BREAKFAST_CLUB].BI_HECHOS_PC ha
	...
	GROUP  BY ha.cod_tiempo	HAVING SUM(ha1.CANTIDAD) < @CANT_TOTAL_VENTAS ) P)

	DECLARE @TIEMPO_COMPRA DECIMAL(18,0) = (SELECT SUM(CANTIDAD * COD_TIEMPO) AS TOTAL FROM [BREAKFAST_CLUB].BI_HECHOS_PC 
	WHERE TIPO_TRANSACCION = 'COMPRA' AND COD_PC = @COD_PC AND COD_SUCURSAL = @COD_SUCURSAL AND COD_TIEMPO <= @MES_ANTERIOR)

	DECLARE @CANT_TOTAL_MES_ANTERIOR DECIMAL(18,0) = (SELECT SUM(CANTIDAD) FROM [BREAKFAST_CLUB].BI_HECHOS_PC
	WHERE TIPO_TRANSACCION = 'COMPRA' AND COD_PC = @COD_PC AND COD_SUCURSAL = @COD_SUCURSAL AND COD_TIEMPO <= @MES_ANTERIOR)
...
BEGIN TRANSACTION
	...
	SELECT 'Compra', AxC.COD_ACCES, S.COD_SUCURSAL, [BREAKFAST_CLUB].obtener_codigo_tiempo(C.FECHA) AS COD_TIEMPO, AxC.PRECIO, SUM(CANTIDAD) AS CANTIDAD, NULL, NULL
	...
	BEGIN TRANSACTION
	INSERT INTO [BREAKFAST_CLUB].[BI_HECHOS_ACCESORIOS]
	SELECT 'Venta', AxF.COD_ACCES, S.COD_SUCURSAL, [BREAKFAST_CLUB].obtener_codigo_tiempo(F.FECHA) AS COD_TIEMPO, AxF.PRECIO, SUM(CANTIDAD) AS CANT, COD_CLIENTE, NULL
...
BEGIN TRANSACTION
	...
	SELECT 'Compra', PxC.COD_PC, COD_RAM, COD_DISCO, COD_GPU, COD_CPU, S.COD_SUCURSAL, NULL, [BREAKFAST_CLUB].obtener_codigo_tiempo(C.FECHA) AS COD_TIEMPO, PxC.PRECIO, SUM(CANTIDAD) AS CANTIDAD,  NULL
	...
	BEGIN TRANSACTION
	INSERT INTO [BREAKFAST_CLUB].[BI_HECHOS_PC] 
	SELECT 'Venta', PxF.COD_PC, COD_RAM, COD_DISCO, COD_GPU, COD_CPU, S.COD_SUCURSAL, F.COD_CLIENTE, [BREAKFAST_CLUB].obtener_codigo_tiempo(F.FECHA) AS COD_TIEMPO, PxF.PRECIO, SUM(CANTIDAD) AS CANTIDAD, NULL
