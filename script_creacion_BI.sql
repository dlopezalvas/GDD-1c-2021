...
CREATE TABLE [BREAKFAST_CLUB].[BI_HECHOS_PC] (
  ...
  [COD_RAM] nvarchar(255) [BREAKFAST_CLUB].[BI_RAM],
  [COD_DISCO] nvarchar(255) [BREAKFAST_CLUB].[BI_DISCO],
  [COD_GPU] decimal (18,0) [BREAKFAST_CLUB].[BI_GPU],
  [COD_CPU] nvarchar(50) [BREAKFAST_CLUB].[BI_CPU],
  [COD_SUCURSAL] decimal (18,0) [BREAKFAST_CLUB].[BI_SUCURSAL],
  [COD_CLIENTE] decimal(18,0) [BREAKFAST_CLUB].[BI_CLIENTE],
  [COD_TIEMPO] decimal(18,0) [BREAKFAST_CLUB].[BI_TIEMPO],
  [PRECIO] decimal(18,2) ,
...
CREATE TABLE [BREAKFAST_CLUB].[BI_HECHOS_ACCESORIOS] (
  [TIPO_TRANSACCION] nvarchar(50),
  [COD_ACCESORIO] nvarchar(255) [BREAKFAST_CLUB].[BI_ACCESORIO],
  [COD_SUCURSAL] decimal (18,0) [BREAKFAST_CLUB].[BI_SUCURSAL],
  [COD_TIEMPO] decimal(18,0) [BREAKFAST_CLUB].[BI_TIEMPO],
  [PRECIO] decimal(18,2) ,
  [CANTIDAD] decimal(18,0),
...
GROUP BY C.FECHA, PC.COD_PC, COD_GPU,  COD_RAM, COD_DISCO, COD_CPU, S.COD_SUCURSAL, PxC.PRECIO
ORDER BY C.FECHA
...
CREATE PROCEDURE [BREAKFAST_CLUB].PRC_SET_STOCK_PC
AS
BEGIN TRANSACTION
...
CREATE PROCEDURE [BREAKFAST_CLUB].PRC_SET_STOCK_ACCESORIOS
AS
BEGIN TRANSACTION
...
BEGIN
	...
	BEGIN
		DECLARE @STOCK_MES_ANTERIOR DECIMAL(18,0) = (SELECT STOCK_DISPONIBLE FROM [BREAKFAST_CLUB].BI_ACCESORIO 
		WHERE COD_TIEMPO = @COD_MES_ANTERIOR AND COD_ACCESORIO = @cod_accesorio AND COD_SUCURSAL = @cod_sucursal)
		DECLARE @CANT_VENTAS_MES_ANTERIOR DECIMAL(18,0) = (SELECT SUM(CANTIDAD) FROM [BREAKFAST_CLUB].BI_ACCESORIO
		...
		DECLARE @CANT_COMPRAS_MES_ACTUAL DECIMAL(18,0) = SELECT SUM(CANTIDAD) FROM [BREAKFAST_CLUB].BI_ACCESORIO
...
BEGIN
	...
	IF (@COD_MES_ANTERIOR != NULL)
	BEGIN
		DECLARE @STOCK_MES_ANTERIOR DECIMAL(18,0) = (SELECT STOCK_DISPONIBLE FROM [BREAKFAST_CLUB].BI_PC 
		WHERE COD_TIEMPO = @COD_MES_ANTERIOR AND COD_PC = @cod_pc AND COD_SUCURSAL = @cod_sucursal)
		DECLARE @CANT_VENTAS_MES_ANTERIOR DECIMAL(18,0) = (SELECT SUM(CANTIDAD) FROM [BREAKFAST_CLUB].BI_PC
		WHERE TIPO_TRANSACCION = 'Venta' AND COD_PC = @cod_pc AND COD_TIEMPO = @COD_MES_ANTERIOR AND COD_SUCURSAL = @cod_sucursal)
		DECLARE @CANT_COMPRAS_MES_ACTUAL DECIMAL(18,0) = SELECT SUM(CANTIDAD) FROM [BREAKFAST_CLUB].BI_PC
