...
AS
BEGIN TRANSACTION
INSERT INTO [BREAKFAST_CLUB].[BI_PC]  ([COD_PC], [ALTO], [ANCHO] , [PROFUNDIDAD], [TIEMPO_PROMEDIO_EN_STOCK], [PRECIO_PROMEDIO_VENTA],[PRECIO_PROMEDIO_COMPRA])
SELECT COD_PC, ALTO, ANCHO, PROFUNDIDAD, [BREAKFAST_CLUB].TIEMPO_PROMEDIO_EN_STOCK_PC(COD_CPU), [BREAKFAST_CLUB].PRECIO_PROMEDIO_VENTAS_PC(COD_PC),
		[BREAKFAST_CLUB].PRECIO_PROMEDIO_COMPRA_PC(COD_PC) FROM [BREAKFAST_CLUB].[PC]
COMMIT;
GO
...
AS
BEGIN TRANSACTION
INSERT INTO [BREAKFAST_CLUB].[BI_ACCESORIO] ([COD_ACCES] , [DESCRIPCION], [TIEMPO_PROMEDIO_EN_STOCK], [PRECIO_PROMEDIO_VENTA], [PRECIO_PROMEDIO_COMPRA])
SELECT COD_ACCES, DESCRIPCION, [BREAKFAST_CLUB].TIEMPO_PROMEDIO_EN_STOCK_ACCESORIO(COD_ACCES), [BREAKFAST_CLUB].PRECIO_PROMEDIO_VENTA_ACC(COD_ACCES),
[BREAKFAST_CLUB].PRECIO_PROMEDIO_VENTA_ACC(COD_ACCES) FROM [BREAKFAST_CLUB].[ACCESORIO]
...
BEGIN TRANSACTION
	...
	BEGIN TRANSACTION
	INSERT INTO [BREAKFAST_CLUB].[BI_HECHOS_VENTAS_ACCESORIOS] ([COD_ACCESORIO], [COD_SUCURSAL], [COD_TIEMPO], [PRECIO], [CANTIDAD_VENDIDA], [COD_CLIENTE])
	SELECT AxF.COD_ACCES, S.COD_SUCURSAL, COD_TIEMPO, AxF.PRECIO, SUM(CANTIDAD), COD_CLIENTE
	...
	GROUP BY S.COD_SUCURSAL, COD_CLIENTE,  AxF.COD_ACCES, AxF.PRECIO, COD_TIEMPO
	ORDER BY COD_TIEMPO
	COMMIT;

	BEGIN TRANSACTION
	UPDATE [BREAKFAST_CLUB].[BI_HECHOS_VENTAS_ACCESORIOS] SET [MAX_STOCK_ANUAL_SUCURSAL] = [BREAKFAST_CLUB].STOCK_MAX_EN_ANIO(COD_ACCESORIO, COD_TIEMPO), 
	[GANANCIAS_MES_SUC] = [BREAKFAST_CLUB].GANANCIAS_ACCESORIOS(COD_TIEMPO, COD_SUCURSAL), 
	[TOTAL_VENDIDO_MES_SUC_ACC] = [BREAKFAST_CLUB].TOTAL_VENDIDO_X_MES_SUC_ACC(COD_TIEMPO, COD_SUCURSAL, COD_ACCESORIO)
	COMMIT;

	BEGIN TRANSACTION
	UPDATE [BREAKFAST_CLUB].[BI_HECHOS_COMPRAS_ACCESORIOS] SET [MAX_STOCK_ANUAL_SUCURSAL] = [BREAKFAST_CLUB].STOCK_MAX_EN_ANIO(COD_ACCESORIO, COD_TIEMPO), 
	[GANANCIAS_MES_SUC] = [BREAKFAST_CLUB].GANANCIAS_ACCESORIOS(COD_TIEMPO, COD_SUCURSAL)
	COMMIT;
...
BEGIN TRANSACTION
	...
	/*COMPRAS ACCESORIOS POR SUCURSAL POR MES */
	BEGIN TRANSACTION 
	INSERT INTO [BREAKFAST_CLUB].[BI_HECHOS_VENTAS_PC] ([COD_PC], [COD_RAM], [COD_DISCO], [COD_GPU], [COD_CPU], [COD_SUCURSAL], [COD_CLIENTE],
    [COD_TIEMPO], [PRECIO], [CANTIDAD_VENDIDA])
	SELECT PxF.COD_PC, COD_RAM, COD_DISCO, COD_GPU, COD_CPU, S.COD_SUCURSAL, F.COD_CLIENTE, COD_TIEMPO, PxF.PRECIO, SUM(CANTIDAD)
	...
	GROUP BY COD_TIEMPO, PxF.COD_PC, COD_GPU,  COD_RAM, COD_DISCO, COD_CPU, S.COD_SUCURSAL, F.COD_CLIENTE, PxF.PRECIO
	ORDER BY COD_TIEMPO
	COMMIT;

	BEGIN TRANSACTION
	UPDATE [BREAKFAST_CLUB].[BI_HECHOS_COMPRAS_ACCESORIOS] SET [GANANCIAS_MES_SUC] = [BREAKFAST_CLUB].GANANCIAS_PC(COD_TIEMPO, COD_SUCURSAL)
	COMMIT;
	
	BEGIN TRANSACTION
	UPDATE [BREAKFAST_CLUB].[BI_HECHOS_VENTAS_PC] SET  [TOTAL_VENDIDO_MES_SUC_PC] = [BREAKFAST_CLUB].TOTAL_VENDIDO_X_MES_SUC_PC(COD_TIEMPO, COD_SUCURSAL, COD_PC),
	[GANANCIAS_MES_SUC] = [BREAKFAST_CLUB].GANANCIAS_PC(COD_TIEMPO, COD_SUCURSAL)
	COMMIT;
...
SELECT C.COD_PC, C.COD_SUCURSAL, MES, ANIO, CANTIDAD_COMPRADA, TOTAL_VENDIDO_MES_SUC_PC
FROM [BREAKFAST_CLUB].BI_TIEMPO LEFT JOIN 
