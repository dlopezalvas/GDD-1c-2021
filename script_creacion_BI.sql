...
BEGIN 
	...
	BEGIN  
		...
		BEGIN
			SET @total_meses = @total_meses + @stock_restante_mes * (@mes_actual - @mes_stock)
		...
		ELSE
		BEGIN
			SET @total_meses = @total_meses + @CANT_VENDIDA_MES * (@mes_actual - @mes_stock)
...
SELECT COD_PC, ROUND(CAST(AVG(TIEMPO_EN_STOCK) AS decimal(18,1)),1) AS STOCK_PROMEDIO_EN_MESES
FROM [BREAKFAST_CLUB].BI_HECHOS_PC
GROUP BY COD_PC
GO
...
SELECT COD_ACCESORIO, ROUND(CAST(AVG(TIEMPO_EN_STOCK) AS decimal(18,1)),1) AS TIEMPO_EN_STOCK_PROMEDIO_EN_MESES
FROM [BREAKFAST_CLUB].BI_HECHOS_ACCESORIOS
GROUP BY COD_ACCESORIO
GO
