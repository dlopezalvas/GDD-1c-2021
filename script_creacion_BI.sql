...
BEGIN
	...
	DECLARE @STOCK_MES_ANTERIOR DECIMAL(18,0)
	DECLARE @CANT_VENTAS_MES_ANTERIOR DECIMAL(18,0)
	IF (@COD_MES_ANTERIOR != NULL)
	BEGIN
		SET @STOCK_MES_ANTERIOR = (SELECT STOCK_DISPONIBLE FROM [BREAKFAST_CLUB].BI_HECHOS_ACCESORIOS 
		WHERE COD_TIEMPO = @COD_MES_ANTERIOR AND COD_ACCESORIO = @cod_accesorio AND COD_SUCURSAL = @cod_sucursal)
		SET @CANT_VENTAS_MES_ANTERIOR = (SELECT SUM(CANTIDAD) FROM [BREAKFAST_CLUB].BI_HECHOS_ACCESORIOS
	...
	ELSE
	BEGIN
		SET @STOCK_MES_ANTERIOR = 0
		SET @CANT_VENTAS_MES_ANTERIOR = 0
	END
...
BEGIN
	...
	IF (@COD_MES_ANTERIOR != NULL)
	BEGIN
		SET @STOCK_MES_ANTERIOR = (SELECT STOCK_DISPONIBLE FROM [BREAKFAST_CLUB].BI_HECHOS_PC 
		WHERE COD_TIEMPO = @COD_MES_ANTERIOR AND COD_PC = @cod_pc AND COD_SUCURSAL = @cod_sucursal)
		SET @CANT_VENTAS_MES_ANTERIOR = (SELECT SUM(CANTIDAD) FROM [BREAKFAST_CLUB].BI_HECHOS_PC
